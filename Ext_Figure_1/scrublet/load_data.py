...
def remove_doublets(adata, doubletf):
  barcodes = adata.obs.index.values
  with open(doubletf, 'r') as f:
    doublets = [b.strip() for b in doubletf]

  is_doublet = np.array([b in doublets for b in barcodes])
  doublet_adata = adata[is_doublet, :]
  adata = adata[np.logical_not(is_doublet), :]
  return adata, doublet_adata
